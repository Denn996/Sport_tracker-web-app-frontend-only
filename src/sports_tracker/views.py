...
def game_list(request):
    ...
    return render(request, 'sports_tracker/game_list.html', {'games': mock_sports_data})

def game_detail(request, game_id):
    """Finds the single requested game from our static list."""
    ...
    mock_sports_data = [
        ...
        {'id': 4, 'name': 'WRC Safari Rally Kenya', 'sport_type': 'Motorsport', 'description': 'World Rally Championship event testing drivers against harsh terrain, deep dust, and unpredictable weather conditions.', 'date_added': 'June 10, 2026'},
        {'id': 5, 'name': 'Wimbledon Grand Slam', 'sport_type': 'Tennis', 'description': 'The oldest and most prestigious tennis tournament in the world, played on traditional grass courts.', 'date_added': 'June 11, 2026'},
        {'id': 6, 'name': 'Kip Keino Classic', 'sport_type': 'Athletics', 'description': 'An annual international track and field meet featuring elite global sprinters and middle-distance runners.', 'date_added': 'June 12, 2026'},
        {'id': 7, 'name': 'World Rugby Sevens Series', 'sport_type': 'Rugby', 'description': 'High-octane, fast-paced international rugby tournament tracking top-tier global teams.', 'date_added': 'June 13, 2026'},
        {'id': 8, 'name': 'Formula 1 Monaco Grand Prix', 'sport_type': 'Motorsport', 'description': 'A tight, historic race requiring absolute precision and making overtaking incredibly challenging.', 'date_added': 'June 14, 2026'},
        {'id': 9, 'name': 'Local Derby Match', 'sport_type': 'Football', 'description': 'A heated regional rivalry match. High emotional stakes, intense crowd energy, and tactical physical battles.', 'date_added': 'June 15, 2026'}
    ]
    
    # Loop through our local list to find the item matching the ID from the URL
    selected_item = None
    for game in mock_sports_data:
        if game['id'] == game_id:
            selected_game = game
            break

    return render(request, 'sports_tracker/game_detail.html', {'game': selected_game})